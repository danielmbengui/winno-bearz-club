...
function(c) {
	...
	function isPhone() {
		...
		let e = $("body");
		isMobile() ? e.addClass("body--mobile") : e.addClass("body--desktop"), isTablet() && e.addClass("body--tablet"), isPhone() && e.addClass("body--phone")
	}();
	var message = {
		show: rua,
		hide: sua,
	...
	function Kxa() {
		for (var e = 0; e < Ixa; e++) Jxa(Hxa.eq(e))
	}

	function Lxa() {
		var e, u;
		Kxa(), e = !1, window.addEventListener("resize", function() {
			clearTimeout(e), e = setTimeout(Kxa, 350)
		}), u = setInterval(function() {
			if ($(".js-tabs-item-list.state-loaded").length === Ixa) clearInterval(u);
			else
				for (var e = $(".js-tabs-item-list:not(.state-loaded)"), t = e.length, n = 0; n < t; n++) {
					...
					r === i.find(".js-tab-content-item.state-loaded").length && (i.addClass("state-loaded"), Jxa(i))
				}
		}, 500)
	}
	submitNoIntegrationForm.init(), window.unicornplatform.subscribeMailchimpForm = {
			...
			for (var e = $(".js-subscribe-zapier-form"), t = e.length, n = 0; n < t; n++) ! function(t) {
				...
				t.on("submit", function(e) {
					e.preventDefault(), $(this), button.showSpinner(r), button.disableSubmit(r), $.ajax({
						...
						}, 3e3), evaluateCodeAfterFormSubmission(u, t), redirectAfterFormSubmission(s, a, l, t.serialize()))
					}).fail(function(e) {
				...
				t.on("submit", function(e) {
					e.preventDefault(), $(this), button.showSpinner(r), button.disableSubmit(r), $.ajax({
						...
						"success" !== e.status ? (message.hide([i, n, o]), message.show(o, 'There was an error. Perhaps you deleted the "ID" column or revoked access to the sheet? Re-adding the integration usually helps.'), button.stopSpinner(r), button.enableSubmit(r)) : (message.hide([i, n, o]), button.showSuccessTick(r), setTimeout(function() {
							...
							button.removeSuccessTick(r), button.enableSubmit(r)
						}, 3e3), evaluateCodeAfterFormSubmission(c, t), redirectAfterFormSubmission(s, a, u, t.serialize()))
					}).fail(function(e) {
	...
	}, window.unicornplatform.subscribeWebhookForm.init(), window.unicornplatform.roadmapScroll = {
		init: function() {
		...
		init: function() {
			Hxa = $(".js-tabs-item-list"), 0 < (Ixa = Hxa.length) && Lxa(), $(document).on("click", ".js-open-tab", function(e) {
	...
	faqToggle.init();
	var openMenu = (Eya = $("body"), {
			bind: function() {
				$(document).on("click", ".js-open-menu", function(e) {
					e.preventDefault();
					var t = $(this).parents(".js-menu");
					$(this).hasClass("state-active-burger") ? Gya(t, $(this)) : (e = t, t = $(this), e.addClass("state-opened-menu"), t.addClass("state-active-burger"), Eya.addClass("state-fixed-body"))
				})
			},
			close: Gya
		}),
	...
	function Gya(e, t) {
		e.removeClass("state-opened-menu"), t.removeClass("state-active-burger"), Eya.removeClass("state-fixed-body")
	}
	openMenu.bind();
	var toggleDropdown = {
		bind: function() {
			...
			$(document).on("click", ".js-toggle-dropdown", function(e) {
				var t = $(this);
				$(this).hasClass("state-opened-dropdown") ? Pya(t) : (Pya(n), t.addClass("state-opened-dropdown"))
			}), $(document).on("click", function(e) {
				!0 === $(e.target).hasClass("js-toggle-dropdown") || 1 === $(e.target).parents(".js-toggle-dropdown").length || Pya(n)
			})
		},
		close: Pya
	};
	...
	var popup = (Mza = {
			...
			openOnPageLoad: function() {
				...
				e && "" !== e && $.featherlight(Nza(e), Mza)
			},
			bind: function() {
				$(document).on("click", ".js-open-popup", function(e) {
					e.preventDefault();
					e = $(this).attr("data-popup-id");
					$(this).featherlight(Nza(e), Mza)
				})
			}
		}),
		Mza, loadMore, Xza, Yza, Zza, _za;
	...
	popup.openOnPageLoad(), popup.bind(), null !== localStorage.getItem("allBlogPosts") && (Xza = $(".js-post-item"), Yza = $(".js-posts-list"), Zza = Xza.length, _za = JSON.parse(localStorage.getItem("allBlogPosts")), loadMore = {
		bind: function() {
			var i = (_za.length - Zza) / 5;
			$(document).on("click", "#js-load-more", function(e) {
				if (e.preventDefault(), 0 < i) {
					for (var t = Zza; t < Zza + 5 && t < _za.length; t++) {
						var n = Xza.clone().eq(0);
						n.attr("href", _za[t].url), "" !== _za[t].og_image_url ? n.find(".js-post-item__img").attr("src", _za[t].og_image_url) : "" !== _za[t].first_image_url && n.find(".js-post-item__img").attr("src", _za[t].first_image_url), n.find(".js-post-item__title").text(_za[t].title), Yza.append(n)
					}
					Zza += 5, i <= 1 && $(".js-load-more-wrapper").hide(), i -= 1
				}
			})
		}
	}, loadMore.bind());
